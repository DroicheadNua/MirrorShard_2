...
### macOS版:  
・アプリケーションが完全に終了している状態でファイルをダブルクリック（または右クリックから開く）しても、ファイルが自動的に開かれない場合があります。  
　アプリケーションを起動した後、画面左上の「開く」ボタン、またはファイルメニューからファイルを選択してください。  

### Linux版（x64, ARM64）:  
・Tauri 2.0の問題として、インライン変換に不具合があります。  
