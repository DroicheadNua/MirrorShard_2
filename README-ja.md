...
ミニマルで没入感の高いデザインが特徴の、AIサポート型ライティングエディタです。  
ソフトウェアフレームワークにTauriを採用、起動の高速化とメモリ消費量の削減に成功。さらに配布バイナリのファイルサイズはわずか10MB以下と、前作（Electron版）より大幅に削減されています。  
テキストファイル対応の軽量アウトラインプロセッサにAI機能を搭載し、カーソル位置からの執筆、コード補完、選択範囲の要約/リライト/翻訳が可能。  
またAIチャットウィンドウ、アイデアプロセッサ、マークダウン／HTMLプレビュー機能、OpenCode連携機能など多彩な機能を実装。クリエイティブ用途やブログ等の執筆、普段遣いのちょっとしたテキスト編集、あるいはOpenCodeを軸としたアプリ開発など、幅広い用途で運用できると思います。  
...
(https://github.com/DroicheadNua/MirrorShard_2/releases/download/v1.5.0/MirrorShard.2_1.5.0_x64_ja-JP.msi)  
[![Mac (Apple Silicon)](https://img.shields.io/badge/Download-Mac_(Apple_Silicon)-green)]  
(https://github.com/DroicheadNua/MirrorShard_2/releases/download/v1.5.0/MirrorShard.2_1.5.0_aarch64.dmg)  
...
何も思いつかないときは、ノードを選択してAIボタンを押せば３つのアイデアが生成される支援機能も  
![MirrorShard_2 Idea_Processor](screenshots/ScreenShot05.jpg)  
また、物語のテンプレート（Story Archetype）と、執筆中の物語の概要に沿ってAIに文章を生成させる「Template Completion」機能も搭載（※画像の例はkimi-k2による生成）  
