...
## 🖼️ AI画像生成（Stable Diffusion / Mistral Agents連携）

MirrorShardはStable DiffusionやMisral Agentsと連携し、テキストからのシームレスな画像生成を実現します。

### 💬 チャットから
- AIに画像生成を依頼（例：「ファンタジー風の森のイラストを作成して」）
- AIが自動的に構造化されたプロンプトを生成
- プロンプトがAPI経由でStable Diffusionに送信
- 生成された画像は保存され、チャット上に直接表示

> プロンプトエンジニアリングは不要――自然言語で指示すれば、あとはAIが自動で処理します。
...
- エディター内の任意のテキストを選択
- 選択した内容を基に画像を生成
- AIがテキストを最適化したプロンプトに変換
- 生成された画像は保存され、カーソル位置にリンクとして挿入
