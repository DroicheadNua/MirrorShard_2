...
* **Wayland環境でのインライン変換対応**:  
  Wayland環境（Niri / GNOME / COSMIC等）で起動された場合、自動的に `GTK_IM_MODULE=wayland` が適用され、エディタ上でのスムーズな直接インライン日本語変換（Fcitx5 / IBus）が機能します。  
* **Over-the-spot入力への移行 (X11・一部の環境)**:  
  X11環境や一部の旧式デスクトップでは、インライン入力ではなく変換中の文字列がIME側の独立した小ウィンドウに表示される「Over-the-spot」入力になる場合があります。  
