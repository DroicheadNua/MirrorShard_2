...
## ⚡️ GPUコンポジットとパフォーマンス
Linux環境において、システム構成（GPUドライバ・セッション種別）に応じて自動的に描画パフォーマンスの最適化が行われます。

* **GPUコンポジットの自動有効化 (高速描画)**
  Intel / AMD のグラフィックボード（Mesaドライバ）環境では、GPUコンポジット描画が自動的に有効化されます。これによりCPU負荷が大幅に低下し、半透明ウィンドウやエフェクトがフリッカー（チラつき）なく快適に動作します。

* **安全性優先の自動フォールバック**
  NVIDIA製グラフィックボード使用時や仮想環境（UTM / VirtualBox / QEMU）では、描画の不安定化を防ぐため、自動的にコンポジットモードが無効化（`WEBKIT_DISABLE_COMPOSITING_MODE=1`）されます。

* **手動オーバーライド（Wayland + NVIDIA環境限定）**
  Wayland環境でNVIDIA製グラフィックボードを使用している場合に限り、設定画面から「Wayland + Nvidia 環境でGPUコンポジットを有効化」を選択することで、GPUコンポジットを手動で有効化できます。環境によっては描画が不安定になる場合があるため、問題が発生した場合はチェックを外して再起動してください。
  なお、ターミナルから `MIRRORSHARD_ENABLE_COMPOSITING=1 mirrorshard2` と実行することでも同様に有効化できます（X11環境や仮想環境では効果がありません）。

### 機能制限

Linux版では、動作の安定性を確保するため以下の機能は全環境で使用できません。  
・スポットライトモード  
・降雪エフェクト  

また、GPUコンポジットが無効化されている場合、追加で以下の機能が使用できなくなります。  
・マークダウン／HTMLプレビュー  
・タイプ音再生  

### 🎨 描画およびその他の問題

* **カラーピッカーの動作不良**: Linux環境のブラウザエンジンの制限により、設定画面（外観タブ等）のカラーピッカーをクリックしても色選択ダイアログが正常に表示されない場合があります。その際は、隣の入力欄に直接カラーコード（例: `#3b3b3b` や `rgba(0,0,0,0.5)`）をテキストで入力して適用してください。  

また、GPUコンポジット無効時などに、以下のような不具合が生じる場合があります。
...
* **コンテキストメニュー（右クリックメニュー）の非表示**: 一部のLinux環境（特にX11環境や仮想マシン上）において、エディタ内で右クリックしてもコンテキストメニューが表示されない場合があります。コピー＆ペースト等はキーボードショートカット（Ctrl+C / Ctrl+V）をご利用ください。
