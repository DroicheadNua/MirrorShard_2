...
「コードエディタモード（Ctrl + K）」の時のみ動作する、プログラミング専用の補完機能です。  
    ...
    制限事項:  
        ...
        ・本機能はローカルLLM専用となります。Geminiでのコード補完はできません。詳しくは次項を御覧ください。  
...
## AIの使い分け  
AIチャットなどで高精度の回答が必要なときはGemini、アイデアプロセッサなどで高速な生成が必要なときはGroq、プライバシー保護が必要な時や、企業製AIの厳格なポリシーに縛られない運用がしたいときにはローカルAIの運用がおすすめです。  


# AI機能使用上の注意  

## ⚠️ AI機能（Gemini・Groq使用時）の利用料金について  
...
ご利用の際は、必ず [Google Gemini APIの料金ページ](https://ai.google.dev/pricing) やGroqの料金プラン（https://console.groq.com/settings/billing/plans）をご確認の上、ご自身の判断でモデルの選択や利用頻度の調整を行ってください。  
