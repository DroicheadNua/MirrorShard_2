...
MirrorShardは「ユーザーが明示的に要求するまでAIに勝手なことはさせない」という設計思想ですので、送信ボタンを押した時点で初めてAIにリクエストが送られます。そのため返答が返ってくるまでに数秒の待ち時間が発生しますが、それはユーザーのプライバシーとPCのリソースを尊重しているからです。  


Q. なんでC++にもJavaにも対応していないのにRustに対応しているんですか？  

A. MirrorShardの1％はRustでできているからです。  